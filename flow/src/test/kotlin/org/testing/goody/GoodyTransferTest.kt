...
class GoodyTransferTest {
    ...
    fun `transfer some goodies`() {
        ...
        val aliceGoodies = transfer[alice] ?: fail("Alice has received no goodies")
    ...
    fun `transfer arrived correctly`() {
        ...
        val aliceGoodies = toAliceGoodies[alice] ?: fail("Alice has received nothing")
        ...
        val bankGoodies = fromAliceGoodies[bankOfGoodies] ?: fail("Bank has received nothing")
    ...
    fun `transfer generates change`() {
        ...
        val bankChange = toAliceGoodies[bankOfGoodies] ?: fail("Bank has received no change")
    ...
    fun `total candy is transferred`() {
        ...
        val toAliceGoodies = toAliceFuture.getOrThrow().tx.outputsOfType<Goody.State>().groupBy(Goody.State::owner)
        log.info("To Alice: output={}", toAliceGoodies)
        val aliceGoodies = toAliceGoodies[alice] ?: fail("Alice has received no goodies")
        assertEquals(1, aliceGoodies.size)
        with(aliceGoodies[0].amount) {
