...
class GoodyMultiIssuerTest {
    ...
    fun `check transfer candy from all issuers`() {
        ...
        val bobGoodies = toBobGoodies[bob] ?: fail("No goodies for Bob")
        assertEquals(2, bobGoodies.size)

        val bankGoodies = bobGoodies.find { it.isIssuedBy(bankOfGoodies) } ?: fail("No goodies from Bank")
        assertEquals(bankCandy.quantity, bankGoodies.amount.quantity)

        val shopGoodies = bobGoodies.find { it.isIssuedBy(shopOfGoodies) } ?: fail("No goodies from Shop")
